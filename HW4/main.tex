...
(a) The explicit Euler method is perhaps the most simple method for numerically solving differential equation, typically introduced in a first-year calculus course.
...
From \fref{prob3}, we can see the performance of our RKF45 with static and adaptive step sizes for three different initial step sizes: $10^{-2},10^{-1},1$.
...
On the other hand, while the steps are accepted initiall for $h_0 = 0.01$, the code makes the subsequent steps larger because such small steps are not necessary to accurately produce the result.
...
\item Under certain conditions the rate of change of the temperature of a body, immersed in a medium whose temperature (kept constant) differs from it, is proportional to the difference in termperature between it and the medium.
    ...
    Let's say you have a cup of coffee at $90 ^{\circ}{\rm C}$ initially ($T_0 = 90$) and its placed in a room where the temperature is $20 ^{\rm circ}{\rm C}$ initially ($T_{M} = 20$).
...
\begin{align}
    T(t) = T_0 + T_{M} ( 1 - e^{-k t} )
,\end{align}
so 
\begin{align}
    T(10~{\rm min}) \approx 47.543 ^{\circ}{\rm C}
.\end{align}
With the RKF45 method with $\epsilon = 10^{-4}$, we obtain $T(10) \approx 47.504^{\circ}{\rm C}$.
Howver, with $\epsilon = 10^{-6}$, we have $T(10) \approx 47.542^{\circ}{\rm C}$.
